...
const PatientHomePage = () => {
  ...
  useEffect(() => {
    const fetchHealthData = async () => {
      try {
        // Get patient ID from localStorage (saved during login)
        const patientId = localStorage.getItem("patientId");

        if (!patientId) {
          console.error(
            "Patient ID not found in localStorage. Please login again."
          );
          return;
        }

        console.log("Fetching health data for patient:", patientId);
        ...
        if (data && data.length > 0) {
          ...
          console.log("No health data available for this patient");
        }
      } catch (error) {
